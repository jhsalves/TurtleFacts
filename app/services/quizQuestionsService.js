...
app.service('QuestionService',['AnswerService',function(AnswerService){
    return {
        getQuestions: function(){
            return quizQuestions;
        },
        setAnswer: function(question,answer){
            var indexQuestion = quizQuestions.indexOf(question);
            if(indexQuestion > -1){
                var item = quizQuestions[indexQuestion];
                item.selected = answer;
                item.correct = AnswerService.isCorrect(indexQuestion,answer);
                quizQuestions[indexQuestion] = item;
            }
        },
        getTotalCorrectAnswers: function(){
            var correctAnswers = quizQuestions.filter(function(item){
                return item.correct === true;
            });
            return correctAnswers.length;
        },
        resetQuizData: function(){
            quizQuestions = quizQuestions.map(function(item){
                item.selected = null;
                item.correct = null;
                return item;
            });
